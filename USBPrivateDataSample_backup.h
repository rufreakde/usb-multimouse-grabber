...
public:
    Device(){
        ...
        this->current_usb_device = 0;
        this->device_name = "Unkown";
    ...
    Device(size_t _device_number, size_t _usb_device, std::string _device_name){
        std::cout << "Create Device - 0.0/0.0" << std::endl;
        this->device_number = _device_number;
        this->current_usb_device = _usb_device;
    ...
    Device(size_t _device_number, size_t _usb_device, std::string _device_name, float _xStart, float _yStart){
    ...
    size_t getUSB_device(){
        return this->current_usb_device;
    };
    std::string getName(){
...
//
//================================================================================================
std::list<Device> getDevicesArray() {
    ...
    while (device_id = IOIteratorNext(io_device_iterator)) {
        ...
        devices.push_back(std::move(current_device));
...
void printDeviceIDs() {
    ...
    io_iterator_t io_device_iterator;
    kern_return_t assembler_kernel_return_value;
    io_service_t current_usb_device;
    ...
    io_name_t device_name;
    /* iterate */
    while (current_usb_device = IOIteratorNext(io_device_iterator)) {
        // do something with current_usb_device, eg. check properties
        // And free the reference taken before continuing to the next item

        IORegistryEntryGetName(current_usb_device, device_name);

        std::cout << "#" << device_number <<  std::endl;
        std::cout << "| io id: "<< "\t" << current_usb_device <<  std::endl;
        std::cout << "| name: " << "\t" << device_name <<  std::endl;
        std::cout << "#-----------------------------------------------#" << std::endl;

        device_number++;
        IOObjectRelease(current_usb_device);
    }
...
{
    kern_return_t   kr;
    deviceDataStruct   *privateDataRef = (deviceDataStruct *) refCon;
...
void DeviceAdded(void *refCon, io_iterator_t iterator)
{
    ...
    while ((usbDevice = IOIteratorNext(iterator))) {
        ...
        deviceDataStruct   *privateDataRef = NULL;
        UInt32          locationID;
        ...
        //TODO CHANGED
        privateDataRef = (deviceDataStruct*)malloc(sizeof(deviceDataStruct));
        bzero(privateDataRef, sizeof(deviceDataStruct));
...
{
    //printDeviceIDs();
    std::list<Device> allDevices = getDevicesArray();
